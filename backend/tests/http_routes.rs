...
const INVENTORY: &[Row] = &[
    ...
    ("POST", "/api/auth/security/register/start", Session, Write, false),
    ("POST", "/api/auth/security/register/finish", Session, Write, false),
    ("POST", "/api/auth/security/verify/start", Session, Write, false),
    ("POST", "/api/auth/security/verify/finish", Session, Write, false),
...
#[test]
fn backend_source_lines_fit_in_140_characters() {
    fn visit(directory: &std::path::Path, long: &mut Vec<std::path::PathBuf>) {
    ...
    for (file, expected) in exceptions {
        let path = root.join(file);
        let count = long.iter().filter(|p| **p == path).count();
        assert_eq!(
            count, expected,
            "reduce the exception budget when wrapping {file}; never increase it"
        );
        long.retain(|p| *p != path);
    }
    assert!(long.is_empty(), "lines over 140 characters: {long:#?}");
}
