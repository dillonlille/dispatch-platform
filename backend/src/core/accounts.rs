...
impl Store {
    ...
    pub fn invite(&self, a: &Auth, dsp: &str, email: &str, role: &str) -> Result<String> {
        ...
        self.platform.exec("INSERT INTO invitations(hash,dsp_id,email,role,expires_at,created_by) VALUES (?,?,?,?,?,?)",params![crypto::sha(&raw),dsp,email.to_lowercase(),role,now()+INVITATION_TTL,s(&a.user,"id")])?;
