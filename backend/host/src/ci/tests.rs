...
impl Fixture {
    fn new() -> Self {
        ...
        let c = context();
        let receipt = json!({"format":1,"repository":crate::REPOSITORY,"workflow":WORKFLOW,"baseRef":"main","runId":5,"attempt":1,"scope":"full","commit":old,"base":c.base,"head":c.head,"tree":c.tree});
...
fn only_trusted_pushes_with_the_actual_source_and_matching_base_can_reuse() {
    ...
    // Only main is trusted, and only for a PR run that recorded main as its target.
    let mut f = Fixture::new();
    f.env.0.insert("GITHUB_REF".into(), "refs/heads/dev".into());
    assert!(f.reuse().unwrap_err().is::<ValidationChanged>());
    let mut f = Fixture::new();
    f.receipt["baseRef"] = "dev".into();
    f.refresh();
    assert!(f.reuse().unwrap_err().is::<ValidationChanged>());
    assert!(Fixture::new().reuse().unwrap());
}
#[test]
fn a_merge_queue_group_promotes_the_gated_build_of_the_pr_run_that_validated_it() {
    ...
    let group = "refs/heads/gh-readonly-queue/main/pr-1-b";
    let f = Fixture::new();
    ...
    // A group queued for another branch than main is not covered by that run, and
    // neither is a group whose validation was revoked meanwhile.
    for (reference, revoke) in [
        ("refs/heads/gh-readonly-queue/dev/pr-1-b", false),
        (group, true),
    ] {
        let mut f = Fixture::new();
        if revoke {
