...
const {parsePeriodKey, validateCode} = require('./timecard-period');

const HEADERS = Object.freeze(['date', 'paycode', 'i1', 'allocation1', 'o1', 'i2', 'allocation2', 'o2', 'hours', 'total_hours', 'amount', 'exception-points', 'waiver', 'comment', 'missing-punch', 'delete']);
...
const RECORD_KEYS = Object.freeze(['additionalRows', 'approvals', 'attestations', 'days', 'employeeCode', 'headers', 'mealWaivers', 'pageTitle', 'periodEnd', 'periodKey', 'periodStart', 'periodTotalHours', 'sourceFormat', 'sourceUrl', 'version', 'weeklyTotals']);
const DAY_KEYS = Object.freeze(['allocation1', 'allocation2', 'comments', 'date', 'dollars', 'exceptionText', 'hours', 'label', 'missingPunch', 'payCode', 'punches', 'totalHours', 'unresolvedSlots', 'waiverChecked']);
const ADDITIONAL_ROW_KEYS = Object.freeze(['allocation1', 'allocation2', 'comments', 'date', 'dollars', 'exceptionText', 'hours', 'payCode', 'punchOrdinals', 'rowClass', 'rowIndex', 'totalHours', 'unresolvedSlots', 'waiverChecked']);
const PUNCH_KEYS = Object.freeze(['actualTime', 'approved', 'changeDetailState', 'changeNote', 'changeOperation', 'changeRequestStatus', 'clockCode', 'clockName', 'comment', 'currentKind', 'currentTime', 'displayTime', 'kind', 'ordinal', 'provenanceAvailable', 'requestedKind', 'requestedTime', 'roundedTime', 'rowIndex', 'slot']);
...
function exactKeys(value, keys) {
  return value && typeof value === 'object' && !Array.isArray(value) && Object.getPrototypeOf(value) === Object.prototype && Object.keys(value).length === keys.length && keys.every(key => Object.prototype.hasOwnProperty.call(value, key));
}

function validateGenericRows(value, code) {
  if (!Array.isArray(value) || value.length > 500) invalid(code);
  for (const row of value) {
    if (!Array.isArray(row) || row.length > 20 || row.some(cell => !bounded(cell, 1000))) invalid(code);
  }
}

function validatePunch(punch, index, seen) {
  const key = punch ? `${punch.rowIndex}:${punch.slot}` : '';
  if (!exactKeys(punch, PUNCH_KEYS) || punch.ordinal !== index + 1 || !Number.isInteger(punch.rowIndex) || punch.rowIndex < 0 || punch.rowIndex > 16 || !SLOTS.includes(punch.slot) || seen.has(key) || !TIME.test(punch.displayTime)) {
    invalid('timecard_punch_invalid');
  }
  if (!bounded(punch.clockName, 200) || !bounded(punch.clockCode, 50) || !bounded(punch.comment, 2000) || typeof punch.provenanceAvailable !== 'boolean' || ![null, 'approved', 'pending', 'rejected'].includes(punch.changeRequestStatus) || typeof punch.approved !== 'boolean' || punch.approved !== (punch.changeRequestStatus === 'approved')) {
    invalid('timecard_provenance_invalid');
  }
  if (punch.provenanceAvailable) {
    if (!KINDS.includes(punch.kind) || !TIME.test(punch.actualTime) || !TIME.test(punch.roundedTime)) invalid('timecard_provenance_invalid');
  } else if (punch.kind !== '' || punch.actualTime !== '' || punch.roundedTime !== '' || punch.clockName !== '' || punch.clockCode !== '' || punch.comment !== '') {
    invalid('timecard_provenance_invalid');
  }
  const nullableKind = value => value === null || KINDS.includes(value);
  const nullableTime = value => value === null || TIME.test(value);
  if (!CHANGE_DETAIL_STATES.includes(punch.changeDetailState) || !nullableKind(punch.currentKind) || !nullableKind(punch.requestedKind) || !nullableTime(punch.currentTime) || !nullableTime(punch.requestedTime) || (punch.changeNote !== null && !bounded(punch.changeNote, 2000)) || (punch.changeOperation !== null && !CHANGE_OPERATIONS.includes(punch.changeOperation))) invalid('timecard_provenance_invalid');
  const emptyDirection = punch.changeOperation === null && punch.currentKind === null && punch.currentTime === null && punch.requestedKind === null && punch.requestedTime === null && punch.changeNote === null;
  if (punch.changeRequestStatus === null) {
    if (punch.changeDetailState !== 'not_applicable' || !emptyDirection) invalid('timecard_provenance_invalid');
    ...
    if (punch.changeOperation === 'add') {
      if (punch.currentKind !== null || punch.currentTime !== null || !requestedPresent) invalid('timecard_provenance_invalid');
    } else if (punch.changeOperation === 'delete') {
      if (!currentPresent || punch.requestedKind !== null || punch.requestedTime !== null) invalid('timecard_provenance_invalid');
    } else if (punch.changeOperation === 'edit') {
      if (!currentPresent || !requestedPresent || punch.currentKind !== punch.requestedKind) invalid('timecard_provenance_invalid');
...
function validateTimecardRecord(value, {employeeCode, period, sourceUrl}) {
  ...
  const headerValid = Array.isArray(value?.headers) && [HEADERS, HEADERS_NO_WAIVER].some(schema => value.headers.length === schema.length && value.headers.every((item, index) => item === schema[index]));

  if (!exactKeys(value, RECORD_KEYS) || value.version !== 2 || value.sourceFormat !== 'paycom-timecard-dom.v2' || value.employeeCode !== employeeCode || value.periodStart !== expected.start || value.periodEnd !== expected.end || value.periodKey !== expected.key || value.sourceUrl !== sourceUrl || value.pageTitle !== 'Timecard Editor') {
    invalid('timecard_identity_invalid');
  }
  if (!headerValid) invalid('timecard_header_invalid');
  if (!Array.isArray(value.days) || value.days.length !== 14) invalid('timecard_day_count_invalid');
  if (!Array.isArray(value.additionalRows) || value.additionalRows.length > 200) invalid('timecard_additional_row_invalid');

  const hasWaiverColumn = value.headers.length === HEADERS.length;
  for (let index = 0; index < 14; index++) {
    const day = value.days[index];
    if (!exactKeys(day, DAY_KEYS) || day.date !== expected.dates[index]) invalid('timecard_date_sequence_invalid');
    ...
    if (!bounded(day.allocation1, 300) || !bounded(day.allocation2, 300)) invalid('timecard_allocation_invalid');
    if (!bounded(day.exceptionText, 2000)) invalid('timecard_exception_invalid');
    if (!numberOrNull(day.hours) || !numberOrNull(day.totalHours) || !numberOrNull(day.dollars)) invalid('timecard_day_number_invalid');
    if (!Array.isArray(day.comments) || day.comments.length > 20 || day.comments.some(item => !bounded(item, 2000))) invalid('timecard_day_comments_invalid');
    if (typeof day.missingPunch !== 'boolean' || !Array.isArray(day.unresolvedSlots) || day.unresolvedSlots.length > 32 || day.unresolvedSlots.some(item => !/^(?:[1-9]|1[0-6]):(?:i1|o1|i2|o2)$|^(?:i1|o1|i2|o2)$/.test(item)) || new Set(day.unresolvedSlots).size !== day.unresolvedSlots.length || day.missingPunch !== (day.unresolvedSlots.length > 0)) invalid('timecard_missing_punch_invalid');
    if (day.waiverChecked !== null && typeof day.waiverChecked !== 'boolean') invalid('timecard_waiver_invalid');
  ...
  for (const row of value.additionalRows) {
    ...
    for (const slot of row.unresolvedSlots) {
      if (!day.unresolvedSlots.includes(`${row.rowIndex}:${slot}`)) invalid('timecard_additional_row_invalid');
    }
  }

  if (!Array.isArray(value.weeklyTotals) || value.weeklyTotals.length !== 2 || value.weeklyTotals.some(item => typeof item !== 'number' || !Number.isFinite(item) || item < 0)) invalid('timecard_weekly_total_invalid');
  if (typeof value.periodTotalHours !== 'number' || !Number.isFinite(value.periodTotalHours) || value.periodTotalHours < 0) invalid('timecard_period_total_invalid');
  if (Math.abs(value.periodTotalHours - value.weeklyTotals.reduce((sum, item) => sum + item, 0)) > 0.011) invalid('timecard_period_total_invalid');
...
function bindRenderedDayDate(value, expectedDate) {
  ...
  return header && typeof expectedDate === 'string' && header.dateText === expectedDate.slice(5).replace('-', '/') ? expectedDate : '';
}

function resolveObservedEmployeeCode(values) {
  if (!Array.isArray(values) || values.length < 1
      || values.some(value => typeof value !== 'string' || !/^[A-Za-z0-9]{4}$/.test(value))) return '';
  const codes = [...new Set(values.map(value => value.toUpperCase()))];
  return codes.length === 1 ? codes[0] : '';
}

function normalizeRequestDetail(observed) {
  const empty = state => ({changeOperation: null, currentKind: null, currentTime: null, requestedKind: null, requestedTime: null, changeNote: null, changeDetailState: state});
  const unavailable = () => empty('unavailable');
  const keys = ['operation', 'currentKind', 'currentTime', 'requestedKind', 'requestedTime', 'note'];
  if (!observed || typeof observed !== 'object' || Array.isArray(observed) || keys.some(key => !Array.isArray(observed[key]))) throw new Error('timecard_provenance_invalid');
  ...
  if (keys.slice(0, 5).some(key => observed[key].length !== 1) || observed.note.length > 1) throw new Error('timecard_provenance_invalid');
  const raw = Object.fromEntries(keys.slice(0, 5).map(key => [key, observed[key][0]]));
  if (Object.values(raw).some(value => typeof value !== 'string' || value.length > 200)) throw new Error('timecard_provenance_invalid');
  const operationAliases = new Map([['add', 'add'], ['edit', 'edit'], ['delete', 'delete'], ['type_change', 'type_change'], ['type change', 'type_change']]);
  const kindAliases = new Map([['IN DAY', 'IN DAY'], ['OUT LUNCH', 'OUT LUNCH'], ['IN LUNCH', 'IN LUNCH'], ['OUT DAY', 'OUT DAY'], ['OUT BREAK', 'OUT LUNCH'], ['IN BREAK', 'IN LUNCH']]);
  ...
  const time = value => value === '' ? null : /^(0[1-9]|1[0-2]):[0-5][0-9] [AP]M$/.test(value.toUpperCase()) ? value.toUpperCase() : undefined;
  ...
  if (!operation || currentKind === undefined || requestedKind === undefined || currentTime === undefined || requestedTime === undefined || typeof note !== 'string' && note !== null || note !== null && (note.length > 2000 || /[\u0000-\u001f\u007f]/.test(note))) throw new Error('timecard_provenance_invalid');
  const complete = operation === 'add' ? currentKind === null && currentTime === null && requestedKind !== null && requestedTime !== null : operation === 'delete' ? currentKind !== null && currentTime !== null && requestedKind === null && requestedTime === null : operation === 'edit' ? currentKind !== null && currentKind === requestedKind && currentTime !== null && requestedTime !== null : currentKind !== null && requestedKind !== null && currentKind !== requestedKind && currentTime !== null && requestedTime !== null;
  if (!complete) throw new Error('timecard_provenance_invalid');
  return {changeOperation: operation, currentKind, currentTime, requestedKind, requestedTime, changeNote: note, changeDetailState: 'complete'};
}
...
function runtimeExtract(config, normalizeRequestDetail, resolveObservedEmployeeCode) {
  ...
  const controlValue = element => {
    ...
    const triggers = Array.from(element.querySelectorAll('a.popoverTrigger.popoverTrigger--text')).filter(e => e.offsetParent !== null);
  ...
  const boundDate = (value, expectedDate) => {
    ...
    return header && header.dateText === expectedDate.slice(5).replace('-', '/') ? expectedDate : '';
  };
  const table = document.querySelector('#tbltimesheet');
  if (!table) return null;
  const identityElements = Array.from(document.querySelectorAll('input[name="firstrefno"],input#firstrefno,[data-firstrefno],[data-employee-code]'));
  const identityValues = identityElements.map(element => clean(element.value || element.getAttribute('data-firstrefno') || element.getAttribute('data-employee-code'))).filter(Boolean);
  ...
  const parsePunch = (element, slot, rowIndex) => {
    ...
    const markerStates = {pcrApproved: 'approved', pcrPending: 'pending', pcrRejected: 'rejected'};
    if (markerTokens.some(token => !Object.prototype.hasOwnProperty.call(markerStates, token)) || new Set(markerTokens).size !== markerTokens.length || markerTokens.length > 1) throw new Error('timecard_pcr_marker_invalid');
    ...
    const directChange = () => {
      if (changeRequestStatus === null) return {changeOperation: null, currentKind: null, currentTime: null, requestedKind: null, requestedTime: null, changeNote: null, changeDetailState: 'not_applicable'};
      const attributes = {
      ...
      for (const candidate of nodes) {
        for (const [key, aliases] of Object.entries(attributes)) for (const alias of aliases) if (candidate.hasAttribute?.(alias)) observed[key].push(clean(candidate.getAttribute(alias)));
        for (const attribute of ['title', 'data-content', 'data-original-title']) {
          ...
          if (typeof rawContent !== 'string' || rawContent.length > 4000) throw new Error('timecard_provenance_invalid');
          const content = decode(rawContent);
          if (content.length > 4000) throw new Error('timecard_provenance_invalid');
          for (const line of content.split(/\n+/).map(clean).filter(Boolean)) {
            for (const [key, aliases] of Object.entries(labels)) for (const alias of aliases) if (line.startsWith(`${alias}:`)) observed[key].push(clean(line.slice(alias.length + 1)));
          }
        }
      }
    ...
    if (!raw) return {ordinal: 0, rowIndex, slot, kind: '', displayTime, actualTime: '', roundedTime: '', clockName: '', clockCode: '', comment: '', provenanceAvailable: false, changeRequestStatus, approved, ...change};
    const text = decode(raw);
    const lines = text.split(/\n+/).map(clean).filter(Boolean);
    const first = lines[0] || '';
    const sourceKind = (first.match(/^(IN DAY|OUT LUNCH|IN LUNCH|OUT DAY|OUT BREAK|IN BREAK)\b/i) || [])[1]?.toUpperCase() || '';
    const kind = sourceKind === 'OUT BREAK' ? 'OUT LUNCH' : sourceKind === 'IN BREAK' ? 'IN LUNCH' : sourceKind;
    const field = name => {
    ...
    return {ordinal: 0, rowIndex, slot, kind, displayTime, actualTime: field('Actual'), roundedTime: field('Rounded'), clockName: match ? clean(match[1]) : clock, clockCode: match ? clean(match[2]) : '', comment: field('Comment'), provenanceAvailable: true, changeRequestStatus, approved, ...change};
  };
  const projectRow = (row, rowIndex) => {
    ...
    return {payCode: controlValue(cell(cells, 'paycode')), allocation1: visible(cell(cells, 'allocation1')), allocation2: visible(cell(cells, 'allocation2')), hours: numeric(visible(cell(cells, 'hours'))), totalHours: numeric(visible(cell(cells, 'total_hours'))), dollars: numeric(visible(cell(cells, 'amount'))), exceptionText: visible(cell(cells, 'exception-points')), waiverChecked: waiver ? Boolean(waiver.checked) : null, comments, unresolvedSlots, punches};
  };
  ...
  const days = dayRows.map((row, index) => {
    ...
    return {date: boundDate(row.children[0]?.textContent, config.period.dates[index] || ''), label: heading?.label || '', payCode: projected.payCode, allocation1: projected.allocation1, allocation2: projected.allocation2, hours: projected.hours, totalHours: projected.totalHours, dollars: projected.dollars, exceptionText: projected.exceptionText, waiverChecked: projected.waiverChecked, comments: projected.comments, missingPunch: projected.unresolvedSlots.length > 0, unresolvedSlots: [...projected.unresolvedSlots], punches: projected.punches};
  });
  const additionalRows = [];
  const nextRowIndex = Array(14).fill(1);
  let dayIndex = -1;
  for (const row of allRows) {
    if (isDay(row)) { dayIndex++; continue; }
    if (isWeekly(row) || dayIndex < 0 || row.children.length !== headers.length || clean(row.children[0]?.textContent) !== '') continue;
    const rowIndex = nextRowIndex[dayIndex]++;
    const projected = projectRow(row, rowIndex);
    const day = days[dayIndex];
    const start = day.punches.length;
    projected.punches.forEach((punch, punchIndex) => { punch.ordinal = start + punchIndex + 1; day.punches.push(punch); });
  ...
  const periodTotalHours = weeklyTotals.every(item => typeof item === 'number') ? Number(weeklyTotals.reduce((a, b) => a + b, 0).toFixed(2)) : null;
  return {version: 2, sourceFormat: 'paycom-timecard-dom.v2', employeeCode: observedEmployeeCode, periodStart: config.period.start, periodEnd: config.period.end, periodKey: config.period.key, sourceUrl: location.href, pageTitle: document.title, headers, days, additionalRows, weeklyTotals, periodTotalHours, approvals: generic('#approvals-table'), attestations: generic('#timecard-attestation-table'), mealWaivers: generic('#meal-waivers-table')};
}

function buildExtractionExpression({employeeCode, period, sourceUrl}) {
  validateCode(employeeCode);
  const config = JSON.stringify({employeeCode, period: parsePeriodKey(period.key), sourceUrl});
  return `(${runtimeExtract.toString()})(${config},${normalizeRequestDetail.toString()},${resolveObservedEmployeeCode.toString()})`;
}

module.exports = {
  HEADERS,
  HEADERS_NO_WAIVER,
  TIME_CARD_VALIDATION_CODES,
  renderedDayHeader,
  bindRenderedDayDate,
  resolveObservedEmployeeCode,
  normalizeRequestDetail,
  buildExtractionExpression,
  validateTimecardRecord
};
