...
function verifyActivationEvidence(options) {
  if (!plain(options) || Object.keys(options).sort().join(',')
      !== 'batchId,clock,collectionPaths,definitionDigest,paycomDatabase,preparationRunId'
      || !plain(options.collectionPaths) || typeof options.paycomDatabase !== 'string'
  ...
  let paycom;
  try {
    manager = new CollectionStore(options.collectionPaths, { readOnly: true });
