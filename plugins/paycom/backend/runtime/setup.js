...
const { ensurePrivateDirectory } = require('../../../../runtime/auth-broker/src/vault');
const { managedInstallationRuntimeEnvironment } = require('dispatch-protocol/paths/runtime-paths');
const { createManagedPaycomActivationRuntime } = require('./activation');
const { managedPaycomDefinition, managedPaycomFirstPublicationRequest } = require('./definition');
const { LocalCollectionAdminPort } = require('../../../../runtime/adapters/local/collection-admin-port');
const { createLocalPaycomActivationEvidencePort } = require('../adapters/activation-evidence');
const { configuration, assertMountBoundary } = require('../../../../runtime/supervisor/src/supervisor');
...
function createContainerPaycomSetup(config, client) {
  ...
  function runtime(input) {
    ...
    return createManagedPaycomActivationRuntime({
      ...
      evidenceVerifier: createLocalPaycomActivationEvidencePort({ environment }),
...
module.exports = { createContainerPaycomSetup };
