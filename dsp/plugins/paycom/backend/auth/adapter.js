...
const { CdpConnection, createTarget } = require('dispatch-sdk/node/cdp');
...
class PaycomAuthError extends Error {
...
function delay(ms, signal) {
  return new Promise((resolve, reject) => {
    ...
    function aborted() { cleanup(); reject(new PaycomAuthError('acquisition_cancelled')); }
...
function authenticatedUrl(value) {
  ...
  const oneEach = unique.size === keys.length;
  if (url.pathname.startsWith(AUTH_PATH_PREFIX)
      && url.pathname !== CLIENT_LANDING_PATH) return oneEach && keys.every(key => key === 'session_nonce') && keys.length <= 1;
  if (![CLIENT_LANDING_PATH, MAIN_MENU_PATH].includes(url.pathname) || !oneEach
      || keys.some(key => !['frmlogin', 'session_nonce'].includes(key)) || keys.length > 2) return false;
...
function exactTimecardSearchUrl(value) {
  ...
  const keys = [...url.searchParams.keys()];
  return new Set(keys).size === keys.length && keys.length <= 2
...
function securityProfileState(snapshot) {
  ...
  if (inputs !== JSON.stringify(['cell-number', 'email', 'work-number'])
  ...
  if (buttons === JSON.stringify(['Continue', 'Not Now', 'Verify', 'Verify', 'Verify']) && !text.includes('Warning')) return 'security_profile_prompt';
  if (buttons === JSON.stringify(['', 'Cancel', 'Continue', 'Continue', 'Not Now', 'Verify', 'Verify', 'Verify'])
      && text.includes('Warning') && text.includes(SECURITY_PROFILE_WARNING)) return 'security_profile_confirmation';
  return 'manual_verification_required';
}

// Reused immediately before optional-profile clicks to catch verification
// controls that appeared after the last observation. No field values are read.
...
function verificationTextMatches(text) {
  ...
  return [
    ['captcha', /captcha/], ['verification_code', /verification code/],
    ['verify_identity', /verify your identity/], ['multi_factor', /multi-factor/], ['one_time_code', /one-time code/],
  ].filter(([, pattern]) => pattern.test(value)).map(([name]) => name);
}

function classifyState(snapshot, { phase = 'observation' } = {}) {
  const result = (state, reason = state) => ({ state, reason });
  ...
  // Visible verification controls must never be dismissed as an optional campaign.
  ...
  if (parsed.pathname === SECURITY_PROFILE_PATH) {
    ...
    // Match the known, optional Not Now flow before interpreting incidental
    // identity-verification wording, as the established Paycom provider does.
  ...
  if (manualChallenge) return result('manual_verification_required', 'additional_verification');
  // Visible fields can precede Paycom's DOMContentLoaded form-token handler.
  if (snapshot.readyState !== 'complete') return result('pending', 'page_loading');
  if (parsed.pathname === SECURITY_QUESTION_PATH) {
    ...
    return indices.length === 2 && new Set(indices).size === 2
      && snapshot.challengeFormCount === 1 && exactSecurityQuestionUrl(snapshot.challengeFormAction)
      && String(snapshot.challengeFormMethod).toUpperCase() === 'POST'
      ? result('security_questions_required') : result('manual_verification_required', 'challenge_layout_changed');
  }
  ...
  if (Array.isArray(snapshot.challenge) && snapshot.challenge.length) return result('manual_verification_required', 'unexpected_challenge');
  if (Array.isArray(snapshot.loginPresent) && snapshot.loginPresent.length === 3 && snapshot.loginPresent.every(Boolean)
      && Array.isArray(snapshot.loginVisible) && snapshot.loginVisible.every(Boolean)
      && snapshot.loginFormCount === 1 && exactLoginUrl(snapshot.url) && exactLoginActionUrl(snapshot.loginFormAction)
      && String(snapshot.loginFormMethod).toUpperCase() === 'POST') return result('logged_out');
  return result('pending', 'page_unrecognized');
}

function classify(snapshot, context) { return classifyState(snapshot, context).state; }

function classificationDiagnostic(state, phase, snapshot, reason) {
  const route = exactSecurityQuestionUrl(snapshot?.url) ? 'security_question'
    : (exactLoginUrl(snapshot?.url) || exactLoginActionUrl(snapshot?.url)) ? 'login'
      : securityProfileUrl(snapshot?.url) ? 'security_profile'
        : authenticatedUrl(snapshot?.url) ? 'application' : 'unknown';
  const evidence = ['primary_credentials_rejected', 'security_answers_rejected'].includes(state)
    ? 'provider_rejection' : 'adapter_check';
  return { phase, route, evidence, reason };
}
...
function stateMetadata(snapshot, diagnostic = null) {
  ...
  const profileInputs = Array.isArray(snapshot?.securityProfile?.inputNames)
    ? snapshot.securityProfile.inputNames.filter(value => typeof value === 'string' && value.length <= 64).slice(0, 16) : [];
  const profileActions = Array.isArray(snapshot?.securityProfile?.buttonTexts)
    ? snapshot.securityProfile.buttonTexts.filter(value => typeof value === 'string' && value.length <= 64).slice(0, 16) : [];
  return {
    ...
    challengeFormMethod: ['GET', 'POST'].includes(String(snapshot?.challengeFormMethod).toUpperCase())
      ? String(snapshot.challengeFormMethod).toUpperCase() : null,
    diagnostic,
  };
}

function loginExpression(credentials) {
  const values = JSON.stringify({ clientCode: credentials.clientCode, username: credentials.username, password: credentials.password }).replace(/</g, '\\u003c');
  return `(()=>{
    const values=${values};
...
function challengeIndices(challenge) {
  ...
  if (indices.some(index => !Number.isInteger(index) || index < 1 || index > 5) || new Set(indices).size !== 2) {
...
function requireNativeInput(browser) {
  if (!browser?.nativeInput || typeof browser.nativeInput.click !== 'function' || typeof browser.nativeInput.type !== 'function') {
    throw new PaycomAuthError('browser_interaction_required');
  }
...
async function submitNativeChallenge(connection, credentials, challenge, browser, signal) {
  ...
  const indices = challengeIndices(challenge);
  // Validate both values before typing either one. Never trim or reinterpret a PIN.
  if (indices.some(index => typeof credentials[`pin${index}`] !== 'string'
      || !/^[\x20-\x7e]{1,64}$/.test(credentials[`pin${index}`]))) throw new PaycomAuthError('manual_verification_required');
  for (const index of indices) {
    ...
    const focused = await connection.evaluate(challengeFocusExpression(challenge, index, { verifyFocus: true }));
    if (focused?.status !== 'native_challenge_field_focused') throw new PaycomAuthError('manual_verification_required');
...
function awaitingProfileControls(snapshot, reason) {
  return securityProfileUrl(snapshot?.url) && snapshot.otpPresent !== true && snapshot.captchaPresent !== true
    && ['security_profile_layout_changed', 'additional_verification'].includes(reason);
}
...
async function waitForState(connection, timeoutMs, accepted, signal, context = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (accepted.has(state) && !profileRendering) return {
        state, snapshot: last, diagnostic: classificationDiagnostic(state, context.phase || 'observation', last, reason),
      };
    } catch (error) {
      if (signal?.aborted) throw new PaycomAuthError('acquisition_cancelled');
    }
  ...
  if (securityProfileUrl(last?.url)) return {
    state: 'manual_verification_required', snapshot: last,
...
const SECURITY_PROFILE_STATES = new Set(['security_profile_prompt', 'security_profile_confirmation']);
...
async function waitForStateChange(connection, previous, signal, context = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      // React can temporarily remove the campaign controls after a click. Wait
      // without clicking again; never grant this grace to another route or to
      // visible verification controls or a provider rejection.
      const profileRendering = awaitingProfileControls(snapshot, reason);
      if (state !== 'pending' && state !== previous && !profileRendering) return {
        state, snapshot, diagnostic: classificationDiagnostic(state, context.phase || 'observation', snapshot, reason),
      };
    } catch (error) {
      if (signal?.aborted) throw new PaycomAuthError('acquisition_cancelled');
    }
  ...
  if (securityProfileUrl(lastSnapshot?.url)) return {
    state: 'manual_verification_required', snapshot: lastSnapshot,
...
async function nativeClick(connection, prepared, expected) {
  if (prepared?.status !== expected || !Number.isFinite(prepared.x) || !Number.isFinite(prepared.y)
      || prepared.x < 0 || prepared.y < 0 || prepared.x > 10_000 || prepared.y > 10_000) throw new PaycomAuthError('manual_verification_required');
  await connection.command('Input.dispatchMouseEvent', { type: 'mouseMoved', x: prepared.x, y: prepared.y });
  await connection.command('Input.dispatchMouseEvent', { type: 'mousePressed', x: prepared.x, y: prepared.y, button: 'left', clickCount: 1 });
  await connection.command('Input.dispatchMouseEvent', { type: 'mouseReleased', x: prepared.x, y: prepared.y, button: 'left', clickCount: 1 });
}
...
async function resolveSecurityProfile(connection, initial, signal) {
  ...
  for (let phase = 0; phase < 3 && SECURITY_PROFILE_STATES.has(current.state); phase++) {
    ...
    const expression = before === 'security_profile_confirmation'
      ? securityProfileConfirmationExpression()
      : proceed ? securityProfileProceedExpression() : securityProfileDismissExpression();
    const expected = before === 'security_profile_confirmation'
      ? 'security_profile_confirmation_ready'
      : proceed ? 'security_profile_proceed_ready' : 'security_profile_dismiss_ready';
    let prepared;
    try { prepared = await connection.evaluate(expression); } catch { throw new PaycomAuthError('manual_verification_required'); }
    try { await nativeClick(connection, prepared, expected); } catch (error) {
...
async function verifyTimecardApplication(connection, initial, credentials, ensureSubmitted, signal, observe = value => value, loginOnly = false, browser = null, observationOnly = false) {
  ...
  for (let phase = 0; phase < 8; phase++) {
    ...
    if (current.state === 'security_questions_required') {
      ...
      try { await submitNativeChallenge(connection, credentials, current.snapshot.challenge, browser, signal); }
      catch (error) {
        observe({ state: 'manual_verification_required', snapshot: current.snapshot,
    ...
    if (current.state === 'authenticated') {
      ...
      if (navigation?.errorText) throw new PaycomAuthError('manual_verification_required');
      current = observe(await waitForState(connection, 45_000, new Set([
        'timecard_application', 'security_questions_required', 'security_profile_prompt',
        'security_profile_confirmation', 'manual_verification_required', 'account_locked', 'logged_out',
...
async function prepareHandoff(browser, sourceTarget, sourceConnection, signal, onState, observedUrl = null) {
  const observed = state => { if (typeof onState === 'function') try { onState(state); } catch {} };
  throwIfAborted(signal);
  ...
  try {
    const clean = await waitForState(cleanConnection, 45_000, new Set(['timecard_application', 'security_questions_required', 'security_profile_prompt', 'security_profile_confirmation', 'manual_verification_required']), signal);
...
// Only a digest leaves the page, kept in broker memory for this handoff. It
// proves the already-entered PINs were not changed while browser control moved.
...
const paycomAdapter = Object.freeze({
  ...
  async prepareBrowserAssistance(browser, { signal } = {}) {
    ...
    const { boundedJson } = require('dispatch-sdk/node/cdp');
    ...
    for (const target of targets.filter(item => item.type === 'page' && parsedPaycomUrl(item.url))) {
      ...
      try {
        ...
        if (classifyState(snapshot).reason === 'additional_verification' && snapshot.captchaPresent === true && snapshot.otpPresent !== true) {
          ...
          selected = { ...target, loaderId: frame.loaderId, challenge: snapshot.challenge, pinFingerprint,
            resumeLogin: !pinFingerprint && (snapshot.loginPresent?.every(Boolean) || snapshot.challenge?.length === 2) };
        }
      } finally { connection.close(); }
    }
    if (!selected) return null;
    // This profile belongs exclusively to Paycom. Ensure the browser tool starts
    // on the challenge instead of a blank or stale sign-in tab.
    for (const target of targets.filter(item => item.type === 'page' && item.id !== selected.id)) {
      await boundedJson(`${browser.endpoint}/json/close/${target.id}`, { signal }).catch(() => {});
    }
    return { pluginId: 'paycom', type: 'captcha', targetId: selected.id,
      loaderId: selected.loaderId, challenge: selected.challenge, pinFingerprint: selected.pinFingerprint, resumeLogin: selected.resumeLogin };
  },
  async completeBrowserAssistance(browser, context, { signal, onState, loginOnly = false, resumeAuthentication } = {}) {
    const { boundedJson } = require('dispatch-sdk/node/cdp');
    const targets = await boundedJson(`${browser.endpoint}/json/list`, { signal });
    const target = targets.find(item => item.type === 'page' && item.id === context.targetId && parsedPaycomUrl(item.url));
    if (!target) throw new PaycomAuthError('manual_verification_required');
    const connection = await CdpConnection.connect(target.webSocketDebuggerUrl, { signal });
    const observed = value => {
      if (typeof onState === 'function') try { onState(value.state, stateMetadata(value.snapshot, value.diagnostic)); } catch {}
      return value;
    };
    const refuseSubmission = () => { throw new PaycomAuthError('manual_verification_required'); };
    try {
      await connection.command('Page.enable');
      let current = observed(await waitForState(connection, 15_000, new Set([
        'authenticated', 'timecard_application', 'security_questions_required', 'security_profile_prompt',
        'security_profile_confirmation', 'manual_verification_required', 'account_locked', 'logged_out',
      ]), signal, { phase: 'recovery_observation' }));
      if (context.resumeLogin && ['logged_out', 'security_questions_required'].includes(current.state)
          && typeof resumeAuthentication === 'function') {
        const frame = (await connection.command('Page.getFrameTree')).frameTree.frame;
        if (frame.loaderId !== context.loaderId) refuseSubmission();
        // The CAPTCHA is gone and this original form had no retained PINs.
        // Return control to the credential-owning broker to finish normal login.
        return await resumeAuthentication();
      }
      if (current.state === 'security_questions_required') {
        const frame = (await connection.command('Page.getFrameTree')).frameTree.frame;
        if (!context.pinFingerprint || frame.loaderId !== context.loaderId
            || await connection.evaluate(RETAINED_PIN_FINGERPRINT) !== context.pinFingerprint) refuseSubmission();
        ...
        throwIfAborted(signal);
        // Complete the same pending form once. Never retype PINs or submit a
        // new/reloaded form after the challenge; ordinary provider handlers run.
      ...
      const verified = await verifyTimecardApplication(connection, current, Object.freeze({}), refuseSubmission,
        signal, observed, loginOnly, browser, true);
  ...
  async inspect(browser, { signal, onState } = {}) {
    throwIfAborted(signal);
    // Reuse a valid session; Paycom redirects expired sessions to its login form.
  ...
  async recover(browser, { signal, onState, loginOnly = false } = {}) {
    ...
    try {
      await connection.command('Page.enable');
      const current = observed(await waitForState(connection, 15_000, new Set([
        'authenticated', 'timecard_application', 'logged_out', 'security_questions_required',
        'security_profile_prompt', 'security_profile_confirmation', 'manual_verification_required',
        'account_locked',
      ]), signal, { phase: 'recovery_observation' }));
      ...
      const verified = await verifyTimecardApplication(
        connection, current, Object.freeze({}), refuseSubmission, signal, observed, loginOnly, browser, true,
      );
      // Setup stops at authenticated login; only collectors need application access and handoff.
      if (loginOnly) return { status: 'authenticated' };
      return prepareHandoff(browser, target, connection, signal, onState, verified.snapshot.url);
    } finally {
      connection.close();
  ...
  async authenticate(browser, credentials, { signal, onSubmit, onState, loginOnly = false } = {}) {
    ...
    try {
      ...
      if (current.state === 'logged_out') {
        ...
        const submitted = await connection.evaluate(loginExpression(credentials));
        if (submitted?.status !== 'submitted') throw new PaycomAuthError('manual_verification_required');
        current = observed(await waitForState(connection, 45_000, new Set([
          'authenticated', 'timecard_application', 'security_questions_required', 'security_profile_prompt',
          'security_profile_confirmation', 'manual_verification_required', 'primary_credentials_rejected', 'account_locked',
...
module.exports = {
  paycomAdapter, PaycomAuthError, LOGIN_URL, LOGIN_ACTION_URL, AUTH_PREFIX, LOGIN_FIELDS, SNAPSHOT,
  SECURITY_QUESTION_PATH, TIMECARD_SEARCH_PATH, TIMECARD_SEARCH_URL, CLIENT_LANDING_PATH, MAIN_MENU_PATH,
  SECURITY_PROFILE_PATH, SECURITY_PROFILE_WARNING, classify, classifyState, loginExpression, challengeExpression,
  challengeFocusExpression, submitNativeChallenge,
  securityProfileUrl, securityProfileState, securityProfileDismissExpression,
  securityProfileConfirmationExpression, securityProfileProceedExpression, resolveSecurityProfile,
  verifyTimecardApplication, waitForState, prepareHandoff, exactLoginUrl, exactLoginActionUrl,
  exactSecurityQuestionUrl, exactTimecardSearchUrl, authenticatedUrl,
};
