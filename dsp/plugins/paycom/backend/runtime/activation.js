...
function createManagedPaycomActivationRuntime(options) {
  ...
  async function startWorkforceSync() {
    ...
    let automatic = true, intervalSeconds = current.data.intervalSeconds;
    if (process.env.DISPATCH_PLUGIN_BACKEND === 'core_v1') {
      const settings = await require('dispatch-sdk/runtime').createFrameworkClient().request('plugin.settings', {pluginId:'paycom',request:{action:'get'}});
      automatic = settings.values.automatic_sync;
      intervalSeconds = settings.values.sync_interval_seconds;
    }
    if (current.data.intervalSeconds !== intervalSeconds) {
      successful(await client.sync.edit(PAYCOM_SYNC_ID, { intervalSeconds, jitterSeconds: Math.min(current.data.jitterSeconds,intervalSeconds-1) }), ['updated'], 'runtime_health_failed');
    }
    if (automatic) successful(await client.sync.start(PAYCOM_SYNC_ID), ['started'], 'runtime_health_failed');
    return Object.freeze({ syncId: PAYCOM_SYNC_ID, intervalSeconds, desiredState: automatic ? 'running' : 'stopped' });
  }
