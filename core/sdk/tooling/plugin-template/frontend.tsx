...
function Page() {
  ...
  if (new URLSearchParams(location.hash.split("?")[1] || "").has("settings"))
    return <PluginSettingsForm pluginId={id} title="__PLUGIN_TITLE__ settings" backHref={`#/${id}`} />;
  if (settings.query.isPending) return <Loading />;
  if (settings.query.error) return <ErrorNotice error={settings.query.error} />;
  return <section className="space-y-5">
    <PageHeading title="__PLUGIN_TITLE__" description="Entries are saved for your DSP."><a href={`#/${id}?settings`}>Settings</a></PageHeading>
