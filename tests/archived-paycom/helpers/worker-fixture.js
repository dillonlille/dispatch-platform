...
cdp.createTarget = async (endpoint, url) => {
  ...
  connection.socket.addEventListener('message', event => {
    ...
    if (selected.pathname === landing) {
      ...
      body += login;
      if (fs.existsSync('/app/captcha-mode') && !/fixture_captcha=solved/.test(request.headers.Cookie || request.headers.cookie || '')) body += '<iframe src="https://captcha-assethost.paycomonline.net/static/hcaptcha.html"></iframe><button id="solveCaptcha" onclick="document.querySelector(\'iframe\').remove();document.cookie=\'fixture_captcha=solved; Path=/; Secure\';this.remove()">Solve fixture</button>';
    }
    ...
    } else if (selected.pathname === pinPath) body += pins;
