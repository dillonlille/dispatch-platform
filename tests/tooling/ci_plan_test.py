...
spec = importlib.util.spec_from_file_location("ci_plan", Path(__file__).parents[2] / "tooling/ci-plan.py")
...
class CiPlanTests(unittest.TestCase):
    ...
    def test_dashboard_scope_is_conservative(self):
        for paths in [["dashboard/src/styles.css", "dashboard/src/archive/styles.css"],
                      ...
                      ["dashboard/src/lib/meal-breaks.ts", "tests/dashboard/meal-breaks.test.ts"],
                      ...
                      ["dashboard/src/lib/format.ts", "tests/dashboard/dashboard-format.test.ts"],
                      ["tests/dashboard/dashboard-structure.test.ts", "tests/dashboard/collection-history.test.ts"]]:
        ...
        plan = json.loads((Path(__file__).parents[2] / "tooling/ci/test-plan.json").read_text())
        self.assertEqual(sorted(ci.DASHBOARD_TESTS), sorted(plan["dashboard"]))
        self.assertIn("...dashboardTests", (Path(__file__).parents[2] / "tooling/ci/checks.ts").read_text())
        ...
        for paths in [[], ["styles.css"], ["dashboard/vite.config.ts"],
                      ...
                      ["shared/contracts/index.ts"], ["shared/contracts/paycom.ts"], ["shared/new-helper.ts"],
                      ["tests/support/support.ts"], ["tests/api/api-auth.test.ts"], ["backend/src/main.rs"],
                      ["tooling/ci/test-plan.json"], ["tooling/ci/test-plan.ts"],
                      ["tests/tooling/test-plan.test.ts", "dashboard/src/main.tsx"],
    ...
    def test_shared_contracts_always_require_full_checks(self):
        for name in ["shared/contracts/meals.ts", "shared/contracts/paycom.ts",
                     "shared/contracts/workforce.ts", "shared/contracts/runtime.ts"]:
            with self.subTest(name=name):
                self.assertEqual(ci.scope([name]), "full")
